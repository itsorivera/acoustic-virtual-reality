...
public:
    ...
    void getHeatMapColor(double value){ //Valor entre 0 y 1
        color arr_col[NUM_COLORS];//arreglo de 4 colores base para el mapa de calor
        ...
        //indices 1 y 2 para encerrar al value con los indices 0 1 2 3 4
        int ind1;
        int ind2;
        ...
        if(value <= 0)
            ind1 = ind2 = 0; //si el valor es -0 entonces indices 1 y 2 = 0
        else if(value >= 1)
            ind1 = ind2 = 3;
        else{
            value = value * (NUM_COLORS-1);  //value = 0.5;
            ind1 = floor(value);//=1
            ind2 = ind1+1;//=1+1=2; el value esta entre ind1 1 y ind2 2
            fracIntermedia = value - double(ind1); //1.5-1=0.5 si es qu esta mas pegado al limite de abajo o arriba
        }
...
//
/*  arreglo [numTri]
    por cada numTri se calculara el distancia con solidAngle()
    por cada numTri se calculara el angulo solido con solidAngle()
    por cada numTri se calculara el tiempo de vuelo

    luego se tendra que aplicar la transicion de energia con la
    ayuda de esos arreglos    
*/
//---------------------------------------------------------------------------
class arrInt {
public:
    int *i;      //Matriz din�mica de enteros
    int I;      //N�mero de filas y columnas
    ...
    void grabarArchivo(char c, int a){
        ...
        FILE *Archivo;
        char buffer[50];
        sprintf(buffer,"Data/aIntReceptor%c_%i.txt",c,a);
        Archivo=fopen(buffer,"w");
...
public:
    ...
    void grabarArchivo(char c, int a){
        ...
        FILE *Archivo;
        char buffer[50];
        sprintf(buffer,"Data/aDouReceptor%c_%i.txt",c,a);
        Archivo=fopen(buffer,"w");
...
public:
        ...
        void clear(){
                ele=0;
                tim=0;
                delete [] energia;//delete de cada uno de los elementos
                energia=NULL;
        };

        //le doy un valor de normalizacion, divido todo lo que tengo de energia entre ese valor
        ...
        //busca el mayor valor de energia
        double maxEne(){
            double maxVal;
            maxVal = 0.0;
            for (int i=0; i<ele; i++) {
    ...
    //grabo los valores de energia en un archivo
